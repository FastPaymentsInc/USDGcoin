...
class NetTest(BitcoinTestFramework):
    ...
    def test_getnodeaddresses(self):
        ...
        self.nodes[0].addpeeraddress(address=ipv6_addr, port=7533)
        ...
        for i in range(10000):
            ...
            self.nodes[0].addpeeraddress(a, 7533)
    ...
    def test_addpeeraddress(self):
        ...
        assert_equal(node.addpeeraddress(address="", port=7533), {"success": False})
        assert_equal(node.getnodeaddresses(count=0), [])

        self.log.debug("Test that adding a valid address succeeds")
        assert_equal(node.addpeeraddress(address="1.2.3.4", port=7533), {"success": True})
        addrs = node.getnodeaddresses(count=0)
        assert_equal(len(addrs), 1)
        assert_equal(addrs[0]["address"], "1.2.3.4")
        assert_equal(addrs[0]["port"], 7533)

        self.log.debug("Test that adding the same address again when already present fails")
        assert_equal(node.addpeeraddress(address="1.2.3.4", port=7533), {"success": False})
