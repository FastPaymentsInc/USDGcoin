...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `usdgd` or `usdg-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
`./`               | `banlist.json`        | Stores the addresses/subnets of banned nodes.
`./`               | `usdg.conf`        | User-defined [configuration settings](usdg-conf.md) for `usdgd` or `usdg-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [usdg.conf](usdg-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
